...
def focus_add_zero(x):
    ...
    new_focus_feature[:, [2, 1], :, :] = new_focus_feature[:, [1, 2], :, :]
    with open('focus_add_channel_642.coe', 'w') as f:
            ...
            for height in range(new_shape[2]):
                for width in range(new_shape[3]):
                    for channel in range(new_shape[1]):
                        ...
                        # if channel <= 3:
                        #     # 3 2 1 0
                        #     f.write('%02x' % int(new_focus_feature[batch][3 - channel][height][width].item()))
                        # else:
                        #     f.write('%02x' % int(new_focus_feature[batch][channel][height][width].item()))

                        if channel <= 3:
                            # 4 5 6 7
                            f.write('%02x' % int(new_focus_feature[batch][channel + 4][height][width].item()))
                        else:
                            # 4 5 6 7
                            # 3 2 1 0
                            # 1 3 5 7
                            ...
                            count += 1
                            f.write(
                                '%02x' % int(new_focus_feature[batch][channel - (2 * count - 1)][height][width].item()))
